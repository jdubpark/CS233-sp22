...
has_visited_position:
    ...
    #
    # NOTE: To optimize movements and window reachability, we restrict the coordinate
    #       to x = 2...38 and y = 17...38
    #
    li      $t1, 0
    li      $t2, 0
    li      $t4, 1
    slt     $t1, $t4, $a0   # $t1 = 1 if 1 < $a0, otherwise $t1 = 0 (from li)
    slti    $t2, $a0, 39    # $t2 = 1 if $a0 < 39, otherwise $t2 = 0 (from li)
    and     $t1, $t1, $t2   # $t1 = 1 if 1 < $a0 < 38, otherwise $t1 = 0

    li      $t2, 0
    li      $t3, 0
    li      $t4, 16
    slt     $t2, $t4, $a1   # $t2 = 1 if 16 < $a1, otherwise $t2 = 0 (from li)
    slti    $t3, $a1, 39    # $t3 = 1 if $a1 < 39, otherwise $t3 = 0 (from li)
    and     $t2, $t2, $t3   # $t2 = 1 if 16 < $a1 < 39, otherwise $t2 = 0

    and     $t1, $t1, $t2,  # $t1 = 1 if $a0,$a1 in the range, otherwise $t1 = 0
    beq     $t1, $0, has_visited_position_out_of_range  # if $t1 = 0 (out-of-range), return 0xFF
...
get_location_data: # returns current map data (16b'0DDDDDDDDDDDDDWI) at tile location ($a0,$a1) = (x,y)
    ...
    # li      $t0, 39 # [0,39]
    # li      $t1, 10
    # blt     $a0, 2, get_location_data_out_of_range
    # bgt     $a0, 38, get_location_data_out_of_range
    # blt     $a1, 17, get_location_data_out_of_range
    # bgt     $a1, 38, get_location_data_out_of_range
...
#
do_powerwash:
    li      $t0, 0x00020000             # (x, y) with no offset and radius = 3
